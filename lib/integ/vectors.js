...
vectors[':'] = function (source, parameters, there) {
    there = ensure(there);
    source = ensure(source, there);
    if (isFn(source.next) && parameters.length > 0 && isFn(parameters[0])) {
        source.next(function (value) {
            there.register(value, parameters[0]);
        });
    }
    return there;
};
...
vectors['_'] = function (source, parameters, there) {
    ...
    var res = there;
    var i = 0;
    if (parameters.length > 0 && isFn(parameters[0])) {
        var cb = parameters[0];
        if (isFn(source.value)) {
            source = source.value();
        }
        if (isFn(source.next)) {
            i = 0;
...
function doNext(source, i, cb, there) {
    ...
    there.join(env);
    delete there.resources['i'];
    delete there.resources['el'];
    return res;
}
