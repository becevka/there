...
function parseNext(string, line, position) {
    ...
    if (type == 'block' || type == 'sequence' || type == 'list') {
        obj.getSequence = function() {
            if(!obj.sequence) {
                obj.sequence = parseNext(obj.value, line, position);
            }
            return obj.sequence;
        }
    }
    if (string) {
        obj.next = parseNext(string, nextLine, nextPosition)
    }
    return obj;
}
...
function parse(program) {
    ...
    return build(parseNext(program, 1, 1), resources);
}
