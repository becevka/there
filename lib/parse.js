...
function parseNext(string, line, position, facet, resources) {
    ...
    var text = getText(next.text, facet.aliases);
    ...
    if (type == 'block' || type == 'sequence' || type == 'list' || type == 'table') {
        obj.getSequence = function () {
            if (!obj.sequence) {
                ...
                obj.sequence = parseNext(obj.value, line, position, f, resources);
                obj.sequence = applyPhrases(f, obj.sequence);
            }
            return obj.sequence;
        }
    } else if(type == 'template') {
        obj.parse = function () {
            return parseNext(obj.value, line, position, {aliases:{}, phrases:{}}, resources);
        }
    }
    if (string) {
        obj.next = parseNext(string, nextLine, nextPosition, facet, resources)
    }
    return obj;
}
...
function getText(text, aliases) {
    ...
    if (alias != null && aliases.hasOwnProperty(text)) {
        return alias;
    }
    return text;
}
...
var getPrepared = function (phrase, facet) {
    ...
    if (!p) {
        var seq = parseNext(phrase, 1, 1, facet, []);
        var replacement = parseNext(facet.phrases[phrase], 1, 1, facet, []);
...
function applyPhrases(facet, sequence) {
    ...
    return {
        parse: function (program) {
            ...
            if (!program) {
                ...
                sequence = parseNext(program, 1, 1, facet, resources);
                sequence = applyPhrases(facet, sequence);
            }
