...
var Evaluate = function (bundle, readline) {
    this.bundle = bundle;
...
Evaluate.prototype._init = function () {
    this.rs = {};
    this.vs = {};
    this._completeBundle();
    var system = this.bundle.system;
    if (system) {
        this.rs = this._addFromArray(this.rs, system.resources);
        this.vs = this._addFromArray(this.vs, system.vectors);
    }
    var user = this.bundle.user;
    if (user) {
        this.rs = this._addFromArray(this.rs, user.resources);
        this.vs = this._addFromArray(this.vs, user.vectors);
    }
};

Evaluate.prototype._addFromArray = function (target, array) {
    if (!array) {
        return target;
    }
...
Evaluate.prototype._completeBundle = function () {
    if (!this.bundle) {
        this.bundle = {};
    }
    ...
    if (!this.bundle.system.vectors) {
        this.bundle.system.vectors = ['vectors'];
    }
    if (!this.bundle.system.resources) {
        this.bundle.system.resources = ['resources'];
    }
    if (!this.bundle.user) {
        this.bundle.user = {};
    }
    if (this.bundle.user.override) {
        if (this.bundle.user.override.vectors) {
            this.bundle.system.vectors = this.bundle.user.override.vectors;
        }
        if (this.bundle.override.resources) {
            this.bundle.system.resources = this.bundle.user.override.resources;
        }
    }
};
