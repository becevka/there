    ...
    switch (type) {
        ...
        case 'resource':
            ...
            if (utils.isFn(r)) {
                ...
                r = r.call(result, obj, there, this);
                r.result = utils.wrapPrimitive(r.result, there);
                return r;
        ...
        case 'word':
            ...
            if (item == null) {
                item = this.globals[value];
                if (item != null) {
                    item = new vector(value, item);
                } else {
                    var c = there.constructor(value);
                    if (c) {
                        item = c(value);
                    } else {
...
var fn = function (sequence, result, obj, there, evaluator) {
    ...
    var v = function (result, params, there, paramNames) {
        ...
        return evaluator.eval(sequence, null, e);
    };
    if (next) {
...
function paramValue(param, there, extend) {
    ...
    if (extend && el instanceof element) {
        el = el.extend();
    }
    return el;
}
